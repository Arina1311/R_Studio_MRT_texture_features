...
run_xgb_selection <- function(data, class_col = "Class", id_col = "Patient_ID", top_n = 10) {
  ...
  # Užtikriname, kad y būtų 0/1 numeric
...
# HDBSCAN
# minPts galima koreguoti. Dažnai pradžiai naudojama 4 arba 5.
set.seed(123)
...
# HDBSCAN triukšmą žymi kaip 0
table(clusters_hdbscan)

# ---------- 6. Vidutinė silueto reikšmė -----------------------------------
...
# ARI:
# 1 - visiškas sutapimas
# 0 - atsitiktiniam priskyrimui artimas sutapimas

# NMI:
# 1 - visiškas informacinis sutapimas
# 0 - nėra bendros informacijos tarp klasterizavimų
...
# ---------- Pacientai pagal klasterius ir tikrą klasę -----------------
...
# AS pacientai su medicininėmis metrikomis
# ------------------------------------------------------------
...
add_clinical_deviations <- function(data) {
  ...
  data |>
    dplyr::mutate(
      ...
      # Pvz. -17 yra blogiau nei -20, todėl -17 + 20 = 3
...
# Pašaliname K-means 2 klasterį
clinical_cluster_as_dev_df <- clinical_cluster_as_dev_df |>
  dplyr::filter(Kmeans != 2) |>
  dplyr::mutate(Kmeans = droplevels(Kmeans))

# Iš naujo perskaičiuojame K-means nuokrypių aprašomąją statistiką
cluster_summary_dev_kmeans <- summarise_deviations_by_cluster(
  data = clinical_cluster_as_dev_df,
  cluster_col = "Kmeans"
)
...
# Tik patologijos grupė, kuriai prieinamos visos klinikinės metrikos
# ============================================================
...
# 1. Paruošiame duomenis koreliacijai
# Naudojame globalios struktūros atrinktus tekstūros požymius
# ir klinikines metrikas tik AS pacientams
...
# 2. Tekstūros požymių ir klinikinių metrikų sąrašai
...
# 3. Funkcija Spirmeno koreliacijoms apskaičiuoti
...
# 4. Apskaičiuojame visas koreliacijas
...
# 5. Atsirenkame tik statistiškai reikšmingas koreliacijas po BH korekcijos
